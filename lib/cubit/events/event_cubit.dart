...
class EventCubit extends Cubit<EventState> {
  ...
  void init(PageCategoryInfo page) async {
    pageID = await page.id!;
    var events = await _database.readAllEventsByIndex(page.id!);
  ...
  Future<EventState> initPageEvents(PageCategoryInfo page) async {
    pageID = page.id!;
    var events = await _database.readAllEventsByIndex(page.id!);
    events.sort((a, b) => b.time.compareTo(a.time));
    return EventState(
  ...
  void addMessageEvent(BuildContext context) async {
    ...
    BlocProvider.of<HomeCubit>(context).updatePage(state.textInput! == '' ? state.textInput! : 'Image File');
  }
